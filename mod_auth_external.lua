...
if not blocking then
	...
	log("info", "External auth in non-blocking mode, yay!")
	waiter, guard = async.waiter, async.guarder();
elseif auth_processes > 1 then
	log("info", "external_auth_processes is greater than 1, but we are in blocking mode - reducing to 1");
	auth_processes = 1;
end
...
function send_query(text)
	...
	if not pty:hasproc() then
		...
		if status and (status ~= "exit" or ret ~= 0) then
			log("info", "Auth process exited unexpectedly with %s %d, restarting", status, ret or 0);
			return nil;
		end
...
function do_query(kind, username, password)
	...
	log("info", "Query auth process for %s", username);
	...
	if not response then
		log("info", "Error while waiting for result from auth process: %s", err or "unknown error");
	...
	elseif (script_type == "ejabberd" and response == "\0\2\0\1") or
		...
		log("info", "Unable to interpret data from auth process, %s",
